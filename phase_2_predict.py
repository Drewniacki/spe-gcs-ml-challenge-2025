...
def predict_phase_2(checkpoint_path: str, well_no: int | None):
    x_init_paths = [
    ...
    target_csvs = [
        "data/raw/phase_2/phase2_blind_test_predictions_1.csv",
        "data/raw/phase_2/phase2_blind_test_predictions_2.csv",
    ]

    # In phase 2 we might want to predict only on a particular well,
    # because we should create one model per well
    if well_no is not None:
        assert isinstance(well_no, int)
        x_init_paths = [x_init_paths[well_no]]
        x_test_paths = [x_test_paths[well_no]]
        target_csvs = [target_csvs[well_no]]
...
if __name__ == "__main__":
    model = "P2-GRU-state0_seq16-hid32-lay2-drp0.2-bch128-d7648193"  # TODO: Change manually
    for split in range(5):
        checkpoint_path = f"runs/{model}/{split}/checkpoints/best_trn_model.pth"
        predict_phase_2(checkpoint_path, well_no=None)
